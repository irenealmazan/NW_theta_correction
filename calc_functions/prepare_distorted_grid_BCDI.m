function [dq_shift,dth_delta_list] = prepare_distorted_grid_BCDI(dth_disp,fly2Danglist,thBragg,index_to_distort,Ry2,Rx)
    ...
    for ii = 1:numel(fly2Danglist)
       ...
       % for jj = 1:numel(index_to_distort)
            if ~isempty(find(ii == index_to_distort))
                ...
                dth_delta_list(ii) = dth_disp(index);
                
                dth = dth_nominal + dth_delta_list(ii);
                
                [dq_shift(ii,:)] = calc_dqshift_for_given_th(dth,ki_o,kf_o,qbragg);
